<x-app-layout>
    ...
    <x-slot name="header">
        <h1 class="mx-4">Update counsellor</h1>
    </x-slot>
    ...
    <div class="flex flex-col items-stretch justify-center m-4">
        ...
        <form method="POST"
            action="{{ route('admin.counsellors.update', ['counsellorDetail' => $counsellorDetail]) }}"
            ...
            <div class="mt-4">
                ...
                <x-input id="name" class="block mt-1 w-full" type="text" name="name"
                    value="{{ $counsellorDetail->user->name }}" required />
            </div>
            ...
            <div class="mt-4">
                ...
                <textarea id="description"
                    ...
                    name="description">{{ $counsellorDetail->description }}</textarea>
            </div>
            ...
            <div class="mt-4">
                ...
                <x-input id="email" class="block mt-1 w-full" type="email" name="email"
                    value="{{ $counsellorDetail->user->email }}" required />
            </div>
                ...
                <div class="flex items-center gap-x-1 mt-1">
                    @if ($speciality->checked)
                        <x-input id="{{ 'specialities.' . $speciality->id }}" class="block" type="checkbox"
                            name="specialities[]" value="{{ $speciality->id }}" checked />
                    @else
                        <x-input id="{{ 'specialities.' . $speciality->id }}" class="block" type="checkbox"
                ...
                <div class="flex items-center gap-x-1 mt-1">
                    @if ($preferLanguage->checked)
                        <x-input id="{{ 'languages.' . $preferLanguage->id }}" class="block" type="checkbox"
                            name="languages[]" value="{{ $preferLanguage->id }}" checked />
                    @else
                        <x-input id="{{ 'languages.' . $preferLanguage->id }}" class="block" type="checkbox"
